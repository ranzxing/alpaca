...
type ProxyHandler struct {
	...
	auth        *authenticator
	onDialError func(string)
}

type proxyFunc func(*http.Request) (*url.URL, error)

func NewProxyHandler(proxy proxyFunc, auth *authenticator, onDialError func(string)) ProxyHandler {
	return ProxyHandler{&http.Transport{Proxy: proxy}, auth, onDialError}
}
...
func (ph ProxyHandler) proxyRequest(w http.ResponseWriter, req *http.Request, auth *authenticator) {
	...
	if err != nil {
		...
		if errors.As(err, &dialErr) && dialErr.address != req.Host {
			...
			ph.onDialError(dialErr.address)
		}
