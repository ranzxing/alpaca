...
func newDirectProxy() ProxyHandler {
	return NewProxyHandler(http.ProxyURL(nil), nil, func(string) {})
}

func newChildProxy(parent *httptest.Server) ProxyHandler {
	findProxy := http.ProxyURL(&url.URL{Host: parent.Listener.Addr().String()})
	return NewProxyHandler(findProxy, nil, func(string) {})
}
